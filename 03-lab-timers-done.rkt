...
#reader(lib "htdp-beginner-reader.ss" "lang")((modname 03-lab-timers-done) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
;;;;; TWO TIMERS ;;;;
...
; process-ftimer : FTimer ... -> ...
; Template for FTimer.
#;
(define (process-ftimer t ...)
  (cond
...
; Extracts the Timer that is current running.
;
; Examples:
(check-expect (ftimer-running (make-running EX-TIMER-0))
              EX-TIMER-0)
...
;
; Examples:
(check-expect (ftimer-replace-running (make-running EX-TIMER-0)
                                    (decr-timer EX-TIMER-0))
              (make-running (decr-timer EX-TIMER-0)))
...
(define (decr-ftimer t)
  (ftimer-replace-running t (decr-timer (ftimer-running t))))
